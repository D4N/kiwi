...
class TestSystemSetup(object):
    ...
    def test_call_non_excutable_config_script(
        self, mock_stat, mock_os_path, mock_watch, mock_command
    ):
        ...
        # pretend that the script is not executable
    ...
    def test_call_excutable_config_script(
        ...
        # pretend that the script is not executable
        mock_stat.return_value = mock.MagicMock()
        mock_stat.return_value.st_mode = 33261
        self.setup.call_config_script()
        print(mock_command.__dict__)
        mock_command.assert_called_once_with(
